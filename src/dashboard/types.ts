...
export interface Fund { code:string; name:string; company:string; managerName:string; managerTenure:number; strategy:string; benchmark:string; expenseRatio:number; aum:number; shares:number; sharesChange20d:number; returnSinceLaunch:number; excessReturn:number; volatility:number; maxDrawdown:number; activeShare:number; holdingsCount:number; turnover20d:number; styleDrift:number; selectionHitRate:number; downsideCapture:number; liquidityCoverage:number; portfolioPE:number; benchmarkPE:number; portfolioROE:number; benchmarkROE:number; earningsGrowth:number; benchmarkGrowth:number; cashWeight:number; top10Weight:number; adv:number; spreadBps:number; premiumDiscount:number; flow20d:number }
